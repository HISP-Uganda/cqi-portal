...
const AllIndicators: FC<AllIndicatorsProps> = ({ rows, dataElementIndex }) => {
  ...
  const { data, isError, isLoading, error, isSuccess } = useAnalyticsStructure(d2, orgUnits, periods);
  return (
    <div>
      {isLoading && <Center className="biggest-height"><Spinner /></Center>}
      {isSuccess && <VStack className="biggest-height" bg="white" overflow="auto" w={window.innerWidth}>
        <Table size="sm">
